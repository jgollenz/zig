...
fn initTLS(phdrs: []elf.Phdr) void {
    ...
    const alloc_size = switch (tls_variant) {
        .VariantI => blk: {
            ...
            l += @sizeOf(CustomData);
            tcb_offset = l;
            l += mem.alignUp(tls_tcb_size, tls_align_factor);
            data_offset = l;
        ...
        .VariantII => blk: {
            var l: usize = 0;
            data_offset = l;
            l += mem.alignUp(tls_data_alloc_size, tls_align_factor);
...
pub fn initStaticTLS(phdrs: []elf.Phdr) void {
    ...
    const tls_area = blk: {
        ...
        const begin_aligned_addr = mem.alignUp(begin_addr, tls_image.alloc_align);
